...
import { PostCargarPlanMinero, getDatos, getAllConceptos, getAllSecuencias, fetchPlanMensual } from '../../api/Axios';
...
const MonthlyPlan = () => {
  ...
  const exportPlanMensualToExcel = async () => {
    ...
    try {
      ...
      if (fecha_ini !== null && fecha_fin !== null && nombre_secuencia !== '') {
        ...
        XLSX.writeFile(workbook, "Plan_Mensual.xlsx");
  ...
  const save = async (e) => {
    ...
    const fecha = selectedDate ? dayjs(selectedDate).format('YYYY-MM-DD') : '';

    try {
      setLoading(true);
      await PostCargarPlanMinero(formData, fecha);
      setFiles([]);
      handleClose();
    ...
    <div>
      ...
      <commonImports.Modal
        ...
        <commonImports.Box sx={style}>
          ...
          <span className="title-form">INGRESO DE EXCELS - PLAN MENSUAL</span>
