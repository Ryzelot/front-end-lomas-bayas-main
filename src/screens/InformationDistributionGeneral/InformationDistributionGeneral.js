  ...
  const exportToExcel = () => {
    ...
    const dataToExport = filteredRows.map(row => {
      ...
      const rowData = {
        ...
        'Fecha Inicio Carga': row.fecha_ini ? formatDate(row.fecha_ini) : "",            // Columna Inicio Carga
        'Fecha Termino Carga': row.fecha_ter ? formatDate(row.fecha_ter) : "",               // Columna Fin Carga
      };
        ...
        rowData['Fecha Inicio Lix'] = row.fecha_lix_inicio ? formatDate(row.fecha_lix_inicio) : "";// Columna Inicio Lixiviación
        rowData['Fecha Termino Lix'] = row.fecha_lix_final ? formatDate(row.fecha_lix_final) : "";     // Columna Fin Lixiviación
        rowData['Fecha Carga Piso Superior'] = row.fecha_p_superior ? formatDate(row.fecha_p_superior) : "";
        rowData['Días de Carga'] = row.dias_cargas;
        rowData['Días Reposo'] = row.dias_reposos;        
        rowData['Días Aporte'] = row.dia_aporte;
        rowData['Ciclo Real'] = row.Ciclo_Reales;
        rowData['Ciclo minimo Requerido'] = row.ciclo_min;
        rowData['Ciclo alcanzable'] = row.Ciclo_Alcanzables;                                 // Columna Ciclo Alcanzable
        rowData['Secado'] = row.secado;
        rowData['Modulos en Riego Actual'] = row.Modulos_En_Riego;
        rowData['Tasa Riego'] = row.tasa_rieg;                                               
        rowData['RL m3/ton'] = row.Rl_valor ? parseFloat(row.Rl_valor).toFixed(3) : 0;         // Columna RL Valor
        ...
        if (nombreSecuencia !== 'HEAP'){
          rowData['%RCuD'] = row.RCuD ? parseFloat(row.RCuD).toFixed(3) : 0;  
        }
        ...
        rowData['%Sulf'] = row.sulf ? parseFloat(row.sulf).toFixed(3) : 0;
        rowData['%Largo'] = row.largo ? parseFloat(row.largo).toFixed(1) : 0;
        rowData['%Ancho'] = row.ancho ? parseFloat(row.ancho).toFixed(1) : 0;
        rowData['Área (m2)']= row.areaxmodulo;                // Columna Área x Módulo
        rowData['Densidad'] = row.densidad ? parseFloat(row.densidad).toFixed(2) : 0;
    ...
    const fileName = `Detalle_${nombreSecuencia}.xlsx`;
          ...
          <commonImports.Table sx={{ minWidth: 400 }} aria-label="customized table" className="table">
            <commonImports.TableHead>
              <commonImports.TableRow>
                ...
                <commonImports.StyledTableCell className="align-element-table sticky-header witdh-information-general">Fecha Inicio</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className="align-element-table sticky-header witdh-information-general">Fecha Término</commonImports.StyledTableCell>
                ...
                {secuencias.find(secuencia => secuencia.id === selectedSecuencia)?.descripcion !== "HEAP" && (
                  <>
                    ...
                    <commonImports.StyledTableCell className='align-element-table sticky-header'>Días Acidulado</commonImports.StyledTableCell> 
                  </>
                )}

                <commonImports.StyledTableCell className="align-element-table sticky-header witdh-information-general">Fecha Inicio Lix </commonImports.StyledTableCell>
                <commonImports.StyledTableCell className="align-element-table sticky-header witdh-information-general">Fecha Termino Lix </commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header witdh-information-general'>Fecha Carga Piso Superior</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Días Cargas</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Días Reposos</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Días Aporte</commonImports.StyledTableCell>   
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Ciclos Reales</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Ciclo Minimo Requerido</commonImports.StyledTableCell>  
                ...
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Secado</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Módulos en Riego Actual</commonImports.StyledTableCell>
                <commonImports.StyledTableCell className='align-element-table sticky-header'>Tasa de Riego</commonImports.StyledTableCell>  
                ...
                <commonImports.TableRow key={row.id}>
                  ...
                  {secuencias.find(secuencia => secuencia.id === selectedSecuencia)?.descripcion !== "HEAP" && (
                    <>
                      ...
                      <commonImports.TableCell>{row.dias_acidulado}</commonImports.TableCell>
                    </>
                  )}
                  ...
                  <commonImports.TableCell>{row.fecha_p_superior ? formatDate(row.fecha_p_superior) : ""}</commonImports.TableCell>
                  <commonImports.TableCell>{row.dias_cargas}</commonImports.TableCell>
                  <commonImports.TableCell>{row.dias_reposos}</commonImports.TableCell>
                  <commonImports.TableCell>{row.dia_aporte}</commonImports.TableCell>
                  <commonImports.TableCell>{row.Ciclo_Reales}</commonImports.TableCell>
                  ...
                  <commonImports.TableCell>{row.Modulos_En_Riego}</commonImports.TableCell>
                  <commonImports.TableCell>{row.tasa_rieg}</commonImports.TableCell>
                  ...
                  <commonImports.TableCell>{row.largo ? parseFloat(row.largo).toFixed(2) : ""}</commonImports.TableCell>
                  <commonImports.TableCell>{row.ancho ? parseFloat(row.ancho).toFixed(2) : ""}</commonImports.TableCell>
                  <commonImports.TableCell>{row.areaxmodulo}</commonImports.TableCell>
                  <commonImports.TableCell>{row.densidad ? parseFloat(row.densidad).toFixed(2) : ""}</commonImports.TableCell>
