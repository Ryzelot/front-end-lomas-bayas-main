...
import { PostCargarForest, getDatos, getAllConceptos, getAllSecuencias, fetchPlanMensual } from '../../api/Axios';
...
const Forecast = () => {
  ...
  const save = async (e) => {
    ...
    const fecha = selectedDate ? dayjs(selectedDate).format('YYYY') : '';

    try {
      setLoading(true);
      await PostCargarForest(formData, fecha);
      setFiles([]);
      handleClose();
    ...
    <div>
      ...
      <commonImports.Modal
        ...
        <commonImports.Box sx={style}>
          ...
          <span className="title-form">INGRESO DE EXCELS - FORCAST</span>
