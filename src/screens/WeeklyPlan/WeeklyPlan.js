...
import { PostCargarPlanSemanal, getDatos, getAllConceptos, getAllSecuencias, fetchPlanMensual } from '../../api/Axios';
...
const WeeklyPlan = () => {
  ...
  const exportPlanMensualToExcel = async () => {
    ...
    try {
      ...
      if (fecha_ini !== null && fecha_fin !== null && nombre_secuencia !== '') {
        ...
        XLSX.writeFile(workbook, "Plan_Semanal.xlsx");
  ...
  const save = async (e) => {
    ...
    try {
      setLoading(true);
      await PostCargarPlanSemanal(formData);
      setFiles([]);
      handleClose();
    ...
    <div>
      ...
      <commonImports.Modal
        ...
        <commonImports.Box sx={style}>
          ...
          <span className="title-form">INGRESO DE EXCELS - PLAN SEMANAL</span>
